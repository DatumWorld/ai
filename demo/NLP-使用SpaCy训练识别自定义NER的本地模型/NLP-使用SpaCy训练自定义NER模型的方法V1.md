...
0. 前提准备：
开发所需要的环境，含预训练模型（自行下载安装）、虚拟环境（python3.12+CUDA12+uv）、训练数据集等，这里跳过此步
...
```
同样的方法，创建验证集，.dev.spacy，参考ner_dev.py
 ```python
import spacy
from spacy.tokens import DocBin
...
<br>亦可使用本目录中的train_fix.py训练（尤其环境报gpu相关错误，但仅有几百条样本更适用CPU快捷看效果时）
5. 测试
比如使用下面代码测试，参考ner_test.py
```python
import spacy
